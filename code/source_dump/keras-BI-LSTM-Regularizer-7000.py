...
#test_df  =pd.read_csv("test_900_feature_df_200_800_2.csv")
...
test_df  =pd.read_csv("test_900_feature_df_200_800_2.csv")
test_more_df  =pd.read_csv("test_900_more_feature_df_200_800_2.csv")


X_test = test_df.drop(['period','powerSetPoint','sigma','delay'],axis=1)
y_test = test_df[['delay']]

X_test_more = test_more_df.drop(['period','powerSetPoint','sigma','delay'],axis=1)
y_test_more = test_more_df[['delay']]

X_test = pd.concat([X_test,X_test_more],axis=0)
y_test = pd.concat([y_test,y_test_more],axis=0)

data_loader = DataLoader(scaler_X.transform(X_test),scaler_y.transform(y_test), params["batch_size"], params["seq_length"],params["input_size"])
X_test, y_test = data_loader.dataset()

predict =  scaler_y.inverse_transform(model.predict(X_test)).flatten()

#predict = predict.astype(int) + (predict - predict.astype(int) > 0.5)
...
df.to_csv('more_result-900-%s.csv' % (saved_model),index=True, header=True)
